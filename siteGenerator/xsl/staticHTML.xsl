 ...
 <!-- =================================================================== -->
    ...
    <xsl:variable name="base-uri">
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:value-of select="'https://hugoye.bethmardutho.org'"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:variable>
    ...
    <xsl:template match="/">
        ...
        <xsl:for-each-group select="$path/child::*" group-by=".">
            <xsl:result-document href="{replace(.,'.xml','.html')}">
                <xsl:message>saved to <xsl:value-of select="$path"/></xsl:message>
                <xsl:choose>
                    <xsl:when test="$fileType = 'HTML'">
        ...
        <xsl:variable name="collectionTemplate">
            <xsl:choose>
                <xsl:when test="$idno != ''">
                    ...
                    <xsl:variable name="templatePath" select="replace(concat($applicationPath,'/siteGenerator/components/',string($collectionValues/@data-template-with),'.html'),'//','/')"/>
                    <!-- Hardcoded page.html value -->
                    <xsl:if test="doc-available(xs:anyURI(concat($applicationPath,'/siteGenerator/components/page.html')))">
                        <xsl:sequence select="document(xs:anyURI(concat($applicationPath,'/siteGenerator/components/page.html')))"/>
                    </xsl:if>
                </xsl:when>
                <xsl:when test="$convert = 'false' and $outputFile != ''">
                ...
                <xsl:when test="$nodes/@data-template-with != ''">
                    ...
                    <xsl:variable name="templatePath" select="replace(concat($applicationPath,'/siteGenerator/components/',substring-after($nodes/@data-template-with,'templates/')),'//','/')"/>
                    <xsl:message>template path <xsl:value-of select="$templatePath"/> </xsl:message>
                ...
                <xsl:choose>
                    <xsl:when test="$pageType = 'HTML'">
                        <xsl:choose>
                            ...
                            <xsl:otherwise>
                                <!--
                                <xsl:message>Troubleshooting::
                                    <xsl:copy-of select="$collectionTemplate"></xsl:copy-of>
                                </xsl:message>
                                -->
                                <xsl:message>Error Can not find matching template for HTML page <xsl:value-of select="replace(concat($applicationPath,'/siteGenerator/components/',substring-after($nodes/@data-template-with,'templates/')),'//','/')"/>
                                </xsl:message>
                            </xsl:otherwise>
                        </xsl:choose>
                    </xsl:when>
                    ...
                    <xsl:when test="$pageType = 'RDF'">
                        <xsl:choose>
                            ...
                            <xsl:otherwise><xsl:message>Error Can not find matching template for TEI page <xsl:value-of select="replace(concat($applicationPath,'/siteGenerator/components/',string($collectionValues/@template),'.html'),'//','/')"/></xsl:message></xsl:otherwise>
                        </xsl:choose>
                    </xsl:when>
                </xsl:choose>
            </xsl:variable>
                                    ...
                                    <div class="interior-content">
                                        ...
                                        <div class="row" xmlns="http://www.w3.org/1999/xhtml">
                                            ...
                                            <div class="col-md-3 col-lg-3 right-menu">
                                                ...
                                                <xsl:call-template name="displayIDs">
                                                    <xsl:with-param name="nodes" select="$nodes"/>
                                                </xsl:call-template>
                                                <span class="badge access-pills"><a href="/open-access.html" style="color:#555">open access <i class="fas fa-unlock-alt"/></a></span>
