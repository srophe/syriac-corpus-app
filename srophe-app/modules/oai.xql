...
(:
 ...
 : Module Version: 1.4
 : Updated: Feb. 16, 2015
 : Record has been update for use by Syriac.org
 : 
 : Date: September, 2007
...
declare function local:oai-metadata($record) {
      ...
      {
        if($metadataPrefix = 'tei') then local:get-TEI($record) 
...
declare function local:get-identifier($record) {
   let $id := replace(string($record/descendant::tei:idno[@type='URI'][starts-with(.,'http://syriaca.org/')][1]),'/tei','')
...
declare function local:oai-get-record() {
    let $_hits := util:eval($_docs)[descendant::tei:idno[@type='URI'][starts-with(.,'http://syriaca.org/')] = $identifier]
    let $record := $_hits
    ...
    return 
        ...
        <GetRecord xmlns="http://www.openarchives.org/OAI/2.0/">
            <record>{
                        ...
                        {
                        let $set := $record//tei:title[@level='m'][1]
                        for $oaiSet in $set
