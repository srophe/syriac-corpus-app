...
declare function timeline:get-birth($data as node()*) as node()?{
    if($data/descendant-or-self::tei:birth) then
        ...
        let $start := if($birth-date/@when) then string($birth-date/@when)
                      ...
                      else if($birth-date/tei:date/@from) then string($birth-date/tei:date/@from)
                      else if($birth-date/tei:date/@notBefore) then string($birth-date/tei:date/@notBefore)
                      else ()
        let $end :=   if($birth-date/@when) then string($birth-date/@when)
                      ...
                      else if($birth-date/tei:date/@to) then string($birth-date/tei:date/@to)
                      else if($birth-date/tei:date/@notAfter) then string($birth-date/tei:date/@notAfter)
                      else ()                    
        return
            <pair type="object">
...
declare function timeline:get-death($data as node()*) as node()?{
       if($data/descendant-or-self::tei:death) then 
        ...
        let $start := if($death-date/@when) then string($death-date/@when)
                      ...
                      else if($death-date/tei:date/@from) then string($death-date/tei:date/@from)
                      else if($death-date/tei:date/@notBefore) then string($death-date/tei:date/@notBefore)
                      else ()
        let $end :=   if($death-date/@when) then string($death-date/@when)
                      ...
                      else if($death-date/tei:date/@to) then string($death-date/tei:date/@to)
                      else if($death-date/tei:date/@notAfter) then string($death-date/tei:date/@notAfter)
                      else () 
        return
            <pair type="object">
                <pair name="startDate" type="string">
...
declare function timeline:get-floruit($data as node()*) as node()*{
   if($data/descendant-or-self::tei:floruit) then 
        ...
        let $start := if($floruit-date/@when) then string($floruit-date/@when)
                      ...
                      else if($floruit-date/tei:date/@from) then string($floruit-date/tei:date/@from)
                      else if($floruit-date/tei:date/@notBefore) then string($floruit-date/tei:date/@notBefore)
                      else ()
        let $end :=   if($floruit-date/@when) then string($floruit-date/@when)
                      ...
                      else if($floruit-date/tei:date/@to) then string($floruit-date/tei:date/@to)
                      else if($floruit-date/tei:date/@notAfter) then string($floruit-date/tei:date/@notAfter)
                      else () 
        return
            <pair type="object">
                    <pair name="startDate" type="string">
...
declare function timeline:get-state($data as node()*) as node()*{
    if($data/descendant-or-self::tei:state) then 
        ...
        let $start := if($state-date/@when) then string($state-date/@when)
                      ...
                      else if($state-date/tei:date/@notBefore) then string($state-date/tei:date/@notBefore)
                      else if($state-date/tei:date/@from) then string($state-date/tei:date/@from)
                      else ()
        let $end :=   if($state-date/@when) then string($state-date/@when)
                      ...
                      else if($state-date/tei:date/@notAfter) then string($state-date/tei:date/@notAfter)
                      else if($state-date/tei:date/@to) then string($state-date/tei:date/@to)
                      else () 
