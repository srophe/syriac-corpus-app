...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:t="http://www.tei-c.org/ns/1.0" xmlns:s="http://syriaca.org" xmlns:tei="http://www.tei-c.org/ns/1.0" xmlns:saxon="http://saxon.sf.net/" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:local="http://syriaca.org/ns" xmlns:x="http://www.w3.org/1999/xhtml" exclude-result-prefixes="xs t s saxon" version="2.0">
 ...
 <!-- ||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||| -->
    ...
    <xsl:template match="/tei:TEI">
        ...
        <xsl:apply-templates select="*[not(self::t:text)]"/>
    ...
    <xsl:template match="t:body">
        <xsl:if test="string-length(descendant::t:desc[not(starts-with(@xml:id,'abstract'))][1]) &gt; 1">
            <div id="description">
                <h3>Brief Descriptions</h3>
                <ul>
                    <xsl:for-each-group select="descendant::t:desc" group-by="if (contains(@xml:lang, '-')=true()) then substring-before(@xml:lang, '-') else @xml:lang">
        ...
        <xsl:if test="descendant::t:person">
            <div id="persnames">
                <xsl:if test="string-length(descendant::t:person/t:desc[@type='abstract' or starts-with(@xml:id, 'abstract-en')][1]                      | descendant::t:person/t:note[@type='abstract']) &gt; 1">
                    <div style="margin-bottom:1em;">
                        <h4>Identity</h4>
                        <xsl:apply-templates select="descendant::t:person/t:desc[@type='abstract' or starts-with(@xml:id, 'abstract-en')][1] | descendant::t:person/t:note[@type='abstract']" mode="abstract"/>
                    </div>
                </xsl:if>
               <!-- NOTE: Need to add Identy description if it exists, When Nathan gets element back to me.  -->
                <p>Names: 
                    <xsl:apply-templates select="descendant::t:person/t:persName[@syriaca-tags='#syriaca-headword' and starts-with(@xml:lang,'syr')]" mode="list">
                        <xsl:sort lang="syr" select="."/>
                    </xsl:apply-templates>
                    <xsl:apply-templates select="descendant::t:person/t:persName[@syriaca-tags='#syriaca-headword' and starts-with(@xml:lang,'en')]" mode="list">
                        <xsl:sort collation="{$mixed}" select="."/>
                    </xsl:apply-templates>
                    <xsl:apply-templates select="descendant::t:person/t:persName[(not(@syriaca-tags) or @syriaca-tags!='#syriaca-headword') and starts-with(@xml:lang, 'syr')]" mode="list">
                        <xsl:sort lang="syr" select="."/>
                    </xsl:apply-templates>
                    <xsl:apply-templates select="descendant::t:person/t:persName[starts-with(@xml:lang, 'ar')]" mode="list">
                        <xsl:sort lang="ar" select="."/>
                    </xsl:apply-templates>
                    <xsl:apply-templates select="descendant::t:person/t:persName[(not(@syriaca-tags) or @syriaca-tags!='#syriaca-headword') and not(starts-with(@xml:lang, 'syr') or starts-with(@xml:lang, 'ar'))]" mode="list">
                        <xsl:sort collation="{$mixed}" select="."/>
                    </xsl:apply-templates>
                </p>
                <br class="clearfix"/>
        ...
        <xsl:if test="descendant::t:place">
            <xsl:if test="descendant::t:place/t:placeName">
                <div id="placenames" class="well">
                    <h3>Names</h3>
                    <xsl:apply-templates select="descendant::t:place/t:placeName[@syriaca-tags='#syriaca-headword' and @xml:lang='syr']" mode="list">
                        <xsl:sort lang="syr" select="."/>
                    </xsl:apply-templates>
                    <xsl:apply-templates select="descendant::t:place/t:placeName[@syriaca-tags='#syriaca-headword' and @xml:lang='en']" mode="list">
                        <xsl:sort collation="{$mixed}" select="."/>
                    </xsl:apply-templates>
                    <xsl:apply-templates select="descendant::t:place/t:placeName[(not(@syriaca-tags) or @syriaca-tags!='#syriaca-headword') and starts-with(@xml:lang, 'syr')]" mode="list">
                        <xsl:sort lang="syr" select="."/>
                    </xsl:apply-templates>
                    <xsl:apply-templates select="descendant::t:place/t:placeName[starts-with(@xml:lang, 'ar')]" mode="list">
                        <xsl:sort lang="ar" select="."/>
                    </xsl:apply-templates>
                    <xsl:apply-templates select="descendant::t:place/t:placeName[(not(@syriaca-tags) or @syriaca-tags!='#syriaca-headword') and not(starts-with(@xml:lang, 'syr') or starts-with(@xml:lang, 'ar'))]" mode="list">
                        <xsl:sort collation="{$mixed}" select="."/>
                    </xsl:apply-templates>
                </div>
            </xsl:if>
            <!-- Related Places -->
            <xsl:if test="descendant::t:place/t:related-places">
                <div id="relations" class="well">
                    <h3>Related Places</h3>
                    <ul>
                        <xsl:apply-templates select="descendant::t:relation" mode="related-place"/>
                    </ul>
                </div>
            </xsl:if>
        </xsl:if>
        <xsl:if test="descendant::t:related-items/descendant::t:relation">
        ...
        <xsl:if test="descendant::t:related-places">
            <div>
                <dl class="dl-horizontal dl-srophe">
                    <xsl:for-each-group select="descendant::t:related-places/t:relation[contains(@uri,'place')]" group-by="@name">
                        <xsl:variable name="desc-ln" select="string-length(t:desc)"/>
                        <xsl:choose>
                            <xsl:when test="not(current-group()/descendant::*:geo)">
                                <dt>&#160;</dt>
        ...
        <xsl:if test="descendant::t:confessions/t:state[@type='confession']">
            <div id="description">
        ...
        <xsl:if test="descendant::t:state[not(@type='confession')]">
        ...
        <!-- Events -->
        <xsl:if test="descendant::t:event[not(@type='attestation')]">
            <div id="event">
                <h3>Event<xsl:if test="count(descendant::t:event[not(@type='attestation')]) &gt; 1">s</xsl:if>
                </h3>
                <ul>
                    <xsl:apply-templates select="descendant::t:event[not(@type='attestation')]" mode="event"/>
                </ul>
            </div>
        </xsl:if>
        
        <!-- Events/attestation -->
        <xsl:if test="descendant::t:event[@type='attestation']">
            <div id="event">
                <h3>Attestation<xsl:if test="count(descendant::t:event[@type='attestation']) &gt; 1">s</xsl:if>
                </h3>
                <ul>
                    <!-- Sorts events on dates, checks first for @notBefore and if not present, uses @when -->
                    <xsl:for-each select="descendant::t:event[@type='attestation']">
        ...
        <xsl:if test="descendant::t:note[not(@type='abstract')]">
            <xsl:for-each-group select="descendant::t:note[not(@type='abstract')]" group-by="@type">
                <h4>
                    <xsl:value-of select="concat(upper-case(substring(current-grouping-key(),1,1)),substring(current-grouping-key(),2))"/>
                </h4>
                <ul>
        ...
        <xsl:if test="descendant::t:srophe-citation">
            <xsl:call-template name="citationInfo"/>
        </xsl:if>
        <!-- Contains: -->
        <xsl:if test="descendant::t:nested-place">
            <div id="contents">
                <h3>Contains</h3>
                <ul>
                    <xsl:for-each select="descendant::t:nested-place">
    ...
    <!-- NOTE: not sure how this should translate -->
        <xsl:if test="descendant::*/@ana ='#syriaca-saint'">
            <div>
                <h3>Lives</h3>
                <p>
    ...
    <!-- Build citation -->
        <xsl:if test="descendant::t:citation">
            <xsl:call-template name="citationInfo"/>
        </xsl:if>
    <!-- Build see also -->
        <xsl:if test="descendant::t:see-also">
    ...
    <!-- Spear factoid, group persons -->
        <xsl:if test="descendant::t:listPerson">
    ...
    <!-- @depreciated Nodes passed from xquery will be matched against this template for formatting -->
    <xsl:template match="t:body" mode="old">
    ...
    <xsl:template match="t:title[parent::t:titleStmt]">
        <h2>
            <xsl:apply-templates/>
        </h2>
    </xsl:template>
