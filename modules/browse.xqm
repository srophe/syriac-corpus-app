...
declare function browse:get-all($node as node(), $model as map(*), $collection as xs:string*, $element as xs:string?){
  map{"browse-data" := data:get-browse-data($collection, 'tei:titleStmt/tei:title') }    
};

(:~
 : Group by Volume or Author. 
:)
declare function browse:group-volumes($node as node(), $model as map(*), $collection as xs:string?){
    ...
    return
        map {"group-by-volume" :=     
                     for $article in $hits
                     let $vol := $hits/descendant::tei:sourceDesc/descendant::tei:biblScope[@type="vol"]
                     group by $vol-facet := $vol
                     let $label := concat('Volume ',$vol-facet/text())
                     order by xs:integer($vol-facet)
                     return
...
declare function browse:group-author($node as node(), $model as map(*), $collection as xs:string?){
    ...
    return
        map {"group-by-author" :=     
                    for $article in $hits
                    let $author := $hits/descendant::tei:titleStmt/descendant::tei:author/tei:name
                    group by $author-facet := $author
                    let $label := concat($author-facet/tei:surname/text(),', ',$author-facet/tei:forename/text())
...
(:
 : Hugoye specific browse for Author
 : Facet by Author (may have to change to group by if they do not like the look) 
 : NOTE: Will have to add paging at some point. 
:)
declare function browse:browse-author($node as node(), $model as map(*), $collection, $sort-options as xs:string*){
