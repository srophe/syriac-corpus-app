...
declare function local:add-page-number($nodes, $start){
    for $node in $nodes
    return
        typeswitch ( $node )
            ...
            case element(tei:pb) return
                let $n := 
                    if($node[@n != '']) then 
                        if($node[@n != $start]) then $start
                        else count($node/preceding::tei:pb) + $start
                    else count($node/preceding::tei:pb) + $start
                return
            ...
            case element() return element { QName("http://www.tei-c.org/ns/1.0", local-name($node)) } {($node/@*, local:add-page-number($node/node(), $start))}                        
            default return local:add-page-number($node/node(), $start)
};
...
let $ed := string($file//tei:pb/@ed)
let $start := if($file//tei:pb[1][@n != '']) then xs:integer($file//tei:pb/@n) else 1
return 
if($name != '') then
   local:add-page-number(local:add-pages($file, $ed), $start)
else 
