...
(: Translate kwic results to be more 'Google like' :)
declare function common:output-kwic($nodes as node()*, $id as xs:string?){
    for $node in $nodes
    return 
        typeswitch($node)
            case text() return ()
            ...
            default return common:output-kwic($node/node(), $id)
};