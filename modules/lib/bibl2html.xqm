...
declare function bibl2html:citation($nodes as node()*) {
    ...
    else if($nodes//tei:teiHeader/tei:fileDesc/tei:sourceDesc/tei:msDesc) then 
        bibl2html:msDesc($nodes/descendant-or-self::tei:sourceDesc/tei:msDesc)
...
declare function bibl2html:monograph($nodes as node()*) {
    ...
    return (
            if(deep-equal($nodes/tei:editor | 
                $nodes/tei:author, $nodes/preceding-sibling::tei:monogr/tei:editor | 
                $nodes/preceding-sibling::tei:monogr/tei:author )) then () 
            else if($nodes/preceding-sibling::tei:analytic) then ' '
            else $persons, 
            tei2html:tei2html($nodes/tei:title[1]),
            if($nodes/preceding-sibling::tei:analytic) then (', ', $persons) else(),
